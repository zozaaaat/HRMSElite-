...
import type { LogData, LogLevel } from "../../shared/types/common";
...
export const log = {
  debug: (message: string, data?: LogData, source?: string) =>
    serverLogger.debug(message, data, source),
  info: (message: string, data?: LogData, source?: string) =>
    serverLogger.info(message, data, source),
  warn: (message: string, data?: LogData, source?: string) =>
    serverLogger.warn(message, data, source),
  error: (message: string, error?: Error | LogData, source?: string) =>
    serverLogger.error(message, error, source),
  api: (
    endpoint: string,
    method: string,
    status?: number,
  ...
  auth: (event: string, userId?: string, data?: LogData) =>
    serverLogger.auth(event, userId, data),
  security: (event: string, ip?: string, data?: LogData) =>
    serverLogger.security(event, ip, data),
  middleware: (name: string, duration?: number, data?: LogData) =>
    serverLogger.middleware(name, duration, data),
  dev: (message: string, data?: LogData, source?: string) =>
    serverLogger.dev(message, data, source),
};
