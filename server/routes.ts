...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (userRole !== "super_admin") {
          return res
            .status(403)
            .json({ message: "Only Super Admin can add companies" });
        }
  ...
  app.get("/api/companies/:companyId", async (req, res) => {
    try {
      ...
      if (
        ...
        (mockCompanies as Record<string, unknown>)[companyId]
      ) {
  ...
  // License routes
  app.get(
    "/api/companies/:companyId/licenses",
    isAuthenticated,
    async (req, res) => {
      try {
        const { companyId } = req.params;
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        const { employeeId } = req.params;
        const leaveData = insertEmployeeLeaveSchema.parse({
          ...req.body,
          employeeId,
        });
  ...
  // Document routes
  app.get(
    "/api/:entityType/:entityId/documents",
    isAuthenticated,
    async (req, res) => {
      try {
        ...
        if (
          !entityType ||
          !["employee", "company", "license"].includes(entityType)
        ) {
  ...
  app.post("/api/government-forms/fill", async (req, res) => {
    try {
      const { formId, employeeId, formData } = req.body as {
        formId: string;
        employeeId: string;
        formData: unknown;
      };
      log.info(
        `Filling form ${formId} for employee ${employeeId}`,
  ...
  app.put("/api/permissions/:employeeId", async (req, res) => {
    try {
      ...
      log.info(
        `Updating permissions for employee ${employeeId}`,
        permissions,
        "ROUTES",
      );
    ...
    try {
      ...
      const responses: Record<string, string> = {
        "ما هو معدل دوران الموظفين الحالي؟":
          "معدل دوران الموظفين الحالي هو 3.2% وهو منخفض بنسبة 15.4% مقارنة بالشهر الماضي، مما يدل على تحسن في رضا الموظفين. هذا المعدل يعتبر صحياً للشركة.",

        "أعطني تحليل رضا الموظفين":
          "رضا الموظفين الحالي 87% مع ارتفاع 5.7%. أعلى الأقسام رضا: تقنية المعلومات (92%)، أقلها: المبيعات (78%). ننصح بتحسين بيئة العمل في قسم المبيعات.",

        "ما هي توقعات النمو للشهر القادم؟":
          "نتوقع نمو بنسبة 3.5% في عدد الموظفين الشهر القادم، مع التركيز على توظيف مطورين وموظفي مبيعات. النمو مدفوع بمشاريع جديدة.",

        "أعطني تقرير الغياب لهذا الشهر":
          "تقرير الغياب لهذا الشهر:\n\n• إجمالي أيام العمل: 22 يوم\n• نسبة الحضور: 92%\n• عدد أيام الغياب: 45 يوم\n• متوسط التأخير: 12 دقيقة\n\nالتوصية: قسم IT لديه أفضل نسبة حضور (95%)",

        "كم رخصة ستنتهي خلال الشهر القادم؟":
          "تحليل التراخيص المنتهية:\n\n⚠️ تراخيص ستنتهي خلال 30 يوم:\n• شركة النيل الأزرق - 15 يوم\n• شركة الاتحاد الخليجي - 30 يوم\n\n📊 إجمالي التراخيص النشطة: 156\n📈 نسبة التجديد المتوقعة: 85%",
      ...
      if (!response) {
        ...
        const partialMatch = Object.keys(responses).find(
          (key) => userMessage.includes(key) ?? key.includes(userMessage),
        );
  ...
  app.post("/api/ai-insights/generate", async (req, res) => {
    try {
      const { type, _companyId } = req.body as {
        type: string;
        _companyId: string;
      };
      ...
      res.json(
        (insights as Record<string, unknown>)[type] ?? {
      ...
      const alerts = [
        {
          ...
          description:
            "معدل دوران الموظفين في قسم المبيعات وصل إلى 8.5% هذا الشهر",
  ...
  app.post("/api/early-warnings/settings", async (req, res) => {
    try {
      const { settings, companyId } = req.body as {
        settings: Record<string, unknown>;
        companyId: string;
      };

      log.info(
        `Updating early warning settings for company ${companyId}`,
        settings,
        "ROUTES",
      );
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch mobile integrations" });
      }
    },
  );

  app.get(
    "/api/mobile/device-registrations",
    isAuthenticated,
    async (req: Request, res: Response) => {
      try {
        const devices = [
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch device registrations" });
      }
    },
  );

  app.get(
    "/api/mobile/stats/:companyId",
    isAuthenticated,
    async (req: Request, res: Response) => {
      try {
        const stats = {
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const aiSummary = {
          ...
          summary: {
            overview:
              "تحليل شامل لبيانات الشركة يظهر نمواً إيجابياً في معظم المؤشرات مع بعض المجالات التي تحتاج إلى تحسين.",
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
            ...
            insights: [
              {
                ...
                title: "تحسين الرواتب",
                description:
                  "إمكانية توفير 180,000 ريال سنوياً من خلال إعادة هيكلة الرواتب",
                impact: "positive",
                confidence: 78,
              },
              {
        ...
        const selectedInsights = type
          ? (insights as Record<string, unknown>)[type as string]
          : insights.employee;
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const selectedPredictions = timeframe
          ? (predictions as Record<string, unknown>)[timeframe as string]
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const selectedReport =
          (reportTypes as Record<string, unknown>)[type] ??
          reportTypes.employee;
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const selectedTrends = metric
          ? (trends as Record<string, unknown>)[metric as string]
          : trends.employee;
