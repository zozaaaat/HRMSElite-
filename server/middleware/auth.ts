...
// JWT Configuration
const JWT_SECRET =
  process.env.JWT_SECRET ?? "hrms-elite-secret-key-change-in-production";
    ...
    if ((req.session as any)?.user) {
      ...
      req.user = {
        ...
        updatedAt: sessionUser.updatedAt
          ? new Date(sessionUser.updatedAt)
          : new Date(),
      };
      return next();
    }
...
export const requireCompanyAccess = (companyIdParam = "companyId") => {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    const companyId =
      req.params[companyIdParam] || (req.query.companyId as string);
    if (!companyId) {
