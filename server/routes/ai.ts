...
const knowledgeBase = {
  hrms: {
    ...
    commonQuestions: {
      "كيف أضيف موظف جديد؟":
        "يمكنك إضافة موظف جديد من خلال صفحة إدارة الموظفين، ثم النقر على زر 'إضافة موظف جديد' وملء النموذج بالمعلومات المطلوبة.",

      "كيف أتحقق من التراخيص المنتهية؟":
        "يمكنك الوصول إلى صفحة إدارة التراخيص والتحقق من قائمة التراخيص التي ستنتهي قريباً أو انتهت بالفعل.",

      "كيف أستخرج تقرير الحضور؟":
        "يمكنك الوصول إلى صفحة التقارير واختيار تقرير الحضور والغياب، ثم تحديد الفترة المطلوبة.",

      "كيف أرفع مستند جديد؟":
        "يمكنك رفع مستند جديد من خلال صفحة إدارة المستندات، ثم النقر على زر 'رفع مستند' واختيار الملف المطلوب.",
    },
    reports: {
      "تقرير الغياب":
        "يظهر تقرير الغياب عدد أيام الغياب لكل موظف خلال فترة محددة مع الأسباب.",
      "تقرير التراخيص":
        "يظهر حالة جميع التراخيص مع تواريخ الانتهاء والتنبيهات.",
...
const checkLLMReady = (
  ...
  if (!localLLM.isInitialized) {
    _res.status(503).json({
      ...
      message:
        "خدمة الذكاء الاصطناعي غير جاهزة حالياً. يرجى المحاولة مرة أخرى لاحقاً.",
    });
    return;
  }
  _next();
  ...
  async (_req: Request, _res: Response) => {
    try {
      ...
      res.status(500).json({
        ...
        message:
          "حدث خطأ أثناء استخراج الكلمات المفتاحية. يرجى المحاولة مرة أخرى.",
      });
    }
  },
);
...
router.post("/chat", checkLLMReady, async (_req: Request, _res: Response) => {
  try {
    ...
    const chatbotFeature = analyticsData.usageStats.popularFeatures.find(
      (f) => f.name === "المساعد الذكي",
    );
    if (chatbotFeature) {
    ...
    const knowledgeData = [
      {
        ...
        content:
          "يمكنك إضافة موظفين جدد، تعديل بياناتهم، حذفهم، ومتابعة أدائهم. النظام يدعم إدارة كاملة لبيانات الموظفين بما في ذلك المعلومات الشخصية، الوظيفية، والمالية.",
      ...
      },
      {
        title: "إدارة الشركات",
        content:
          "النظام يدعم إدارة متعددة الشركات. يمكنك إضافة شركات جديدة، تعديل معلوماتها، وإدارة العلاقة بين الشركات والموظفين.",
      ...
      },
      {
        ...
        content:
          "متابعة تراخيص العمل والانتهاء. النظام ينبهك للتراخيص التي ستنتهي قريباً ويساعدك في تجديدها في الوقت المناسب.",
      ...
      },
      {
        ...
        content:
          "تسجيل ومتابعة الحضور والغياب للموظفين. النظام يحسب ساعات العمل، التأخير، والإجازات.",
      ...
      },
      {
        title: "إدارة الرواتب",
        content:
          "حساب وإدارة رواتب الموظفين. النظام يدعم الخصومات، الإضافات، والمكافآت مع إمكانية توليد تقارير الرواتب.",
      ...
      },
      {
        ...
        content:
          "رفع وإدارة المستندات المهمة مثل العقود، الشهادات، والوثائق الرسمية. النظام يدعم تصنيف المستندات والبحث فيها.",
      ...
      },
      {
        ...
        content:
          "توليد تقارير مفصلة عن جميع العمليات. النظام يوفر تحليلات ذكية ومؤشرات الأداء لمساعدتك في اتخاذ القرارات.",
      ...
      },
      {
        ...
        content:
          "مساعد ذكي للاستعلامات والتحليل. النظام يستخدم الذكاء الاصطناعي لتقديم توصيات ذكية وتحليلات متقدمة.",
...
function processChatMessage(message: string): string {
  ...
  for (const [question, answer] of Object.entries(
    knowledgeBase.hrms.commonQuestions,
  )) {
    if (
  ...
  if (
    lowerMessage.includes("ميزة") ||
    lowerMessage.includes("feature") ||
    lowerMessage.includes("ما يمكن")
  ) {
  ...
  if (
    lowerMessage.includes("ترخيص") &&
    (lowerMessage.includes("ينتهي") || lowerMessage.includes("expire"))
  ) {
  ...
  // Default response
  return (
    "مرحباً! أنا المساعد الذكي لـ HRMS Elite. يمكنني مساعدتك في:\n" +
...
function generateAnalyticsData(
    ...
    usageStats: {
      ...
      popularFeatures: [
        ...
        { name: "تحليل المشاعر", count: 289 + Math.floor(Math.random() * 25) },
        {
          ...
          count: 160 + Math.floor(Math.random() * 20),
        },
        {
          name: "المساعد الذكي",
          count:
            analyticsData.usageStats.popularFeatures.find(
              (f) => f.name === "المساعد الذكي",
            )?.count ?? 0,
        },
      ],
  ...
  if (category !== "all") {
    baseData.recommendations = baseData.recommendations.filter(
