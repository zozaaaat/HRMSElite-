...
export function registerEmployeeRoutes (app: Express) {
  ...
  app.post('/api/employees/:employeeId/leaves', isAuthenticated, async (req, res) => {
    ...
    try {
      ...
      const leaveData = {...req.body as Record<string, unknown>, employeeId};
   ...
   * }
   */
  app.post('/api/employees/:employeeId/deductions',
   isAuthenticated,
   requireRole(['super_admin',
   'company_manager']),
   async (req,
   res) => {
    ...
    try {
      ...
      const deductionData = {
        ...
        employeeId,
        'processedBy': req.user && 'userId' in req.user ? req.user.userId : undefined
      };
      ...
      try {
        const deduction = await storage.createDeduction(
          result.data,
          typeof req.user === 'object' && req.user && 'userId' in req.user && typeof req.user.userId === 'string'
            ? req.user.userId
            : "system"
        );
   ...
   * }
   */
  app.post('/api/employees/:employeeId/violations',
   isAuthenticated,
   requireRole(['super_admin',
   'company_manager']),
   async (req,
   ...
   typeof req.user === 'object' && req.user && 'id' in req.user && typeof req.user.id === 'string'
     ? req.user.id
     : "system");
