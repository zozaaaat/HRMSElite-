...
router.get("/quality-metrics", async (req, res) => {
  try {
    ...
    log.error(
      "Error fetching quality metrics:",
...
router.post("/quality-metrics/run", async (req, res) => {
  try {
    ...
    log.error(
      "Error running quality monitoring:",
      error instanceof Error ? error : new Error(String(error)),
    );
    res.status(500).json({
  ...
  for (const line of lines) {
    ...
    if (line.includes("TypeScript:")) {
      ...
      if (match?.[1] && match?.[2]) {
        ...
        results.typescript.status =
          results.typescript.errors === 0 ? "pass" : "fail";
      }
    }
    ...
    if (line.includes("Lighthouse:")) {
      const match = line.match(
        /Lighthouse: Performance (\d+)%, Accessibility (\d+)%/,
      );
      if (match?.[1] && match?.[2]) {
    ...
    if (line.includes("Coverage:")) {
      ...
      if (match?.[1] && match?.[2]) {
        ...
        results.coverage.status =
          results.coverage.lines >= 80 ? "pass" : "fail";
      }
    }
