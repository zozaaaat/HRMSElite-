  ...
  try {
    ...
    if (companyId) {
      ...
      if (!currentCompany) {
        return res
          .status(403)
          .json({ message: "No access to specified company" });
      }
    }
    ...
    const sessionUser: SessionUser = {
      ...
      claims:
        typeof user.claims === "string"
          ? (JSON.parse(user.claims) as Record<string, unknown>)
          : user.claims,
      createdAt: user.createdAt,
      updatedAt: user.updatedAt,
      companyId: user.companyId,
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const userCompanies = await storage.getUserCompanies(userId);
      const targetCompany = userCompanies.find(
        (company) => company.id === companyId,
      );
...
router.post("/reset-password", async (req: Request, res: Response) => {
  try {
    ...
    if (!user) {
      return res
        .status(400)
        .json({ message: "Token غير صالح أو منتهي الصلاحية" });
    }

    log.info(
      `Password reset successful for user ${user.id}`,
      undefined,
      "AUTH",
    );
