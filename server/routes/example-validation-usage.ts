...
const employeeUpdateSchema = z.object({
  ...
  position: z
    .string()
    .min(2, "المنصب يجب أن يكون على الأقل حرفين")
    .max(100, "المنصب طويل جداً"),
  department: z
    .string()
    .min(2, "القسم يجب أن يكون على الأقل حرفين")
  ...
  hireDate: z
    .string()
    .regex(/^\d{4}-\d{2}-\d{2}$/, "تاريخ التوظيف يجب أن يكون بصيغة YYYY-MM-DD"),
});
...
const documentUploadSchema = z.object({
  ...
  entityType: z.enum(["employee", "company", "license"], {
    errorMap: () => ({
      message: "نوع الكيان يجب أن يكون: employee, company, أو license",
    }),
  }),
  name: z
    .string()
    .min(1, "اسم المستند مطلوب")
    .max(200, "اسم المستند طويل جداً"),
  type: z.enum([
    "passport",
    "residence",
    "license",
...
const leaveRequestSchema = z.object({
  ...
  startDate: z
    .string()
    .regex(/^\d{4}-\d{2}-\d{2}$/, "تاريخ البداية يجب أن يكون بصيغة YYYY-MM-DD"),
  endDate: z
    .string()
    .regex(/^\d{4}-\d{2}-\d{2}$/, "تاريخ النهاية يجب أن يكون بصيغة YYYY-MM-DD"),
...
export function registerExampleValidationRoutes(app: Express) {
  ...
  app.put(
    ...
    validateInput.params(
      z.object({ id: z.string().min(1, "معرف الموظف مطلوب") }),
    ),
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        const { name, industryType, location, isActive, page, limit } =
          req.query;
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const licenses = companyId
          ? await storage.getCompanyLicenses(companyId as string)
          : [];
