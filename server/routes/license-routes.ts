...
export function registerLicenseRoutes(app: Express) {
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        log.error(
          "Error fetching company licenses:",
          error instanceof Error ? error : { error },
        );
  ...
  app.patch(
    ...
    async (req: Request, res: Response) => {
      try {
        const { licenseIds, status } = req.body as {
          licenseIds: string[];
          status: string;
        };
