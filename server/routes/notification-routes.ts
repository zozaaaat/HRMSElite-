...
router.get<{}, Notification[] | { error: string }, {}, GetNotificationsQuery>(
  "/",
  async (req, res) => {
    try {
...
router.get<{}, number | { error: string }>(
  "/unread-count",
  async (req, res) => {
    try {
      const userId = req.user?.id;
      ...
      log.error(
        "خطأ في الحصول على عدد الإشعارات غير المقروءة:",
        error instanceof Error ? error : new Error(String(error)),
      );
      res.status(500).json({ error: "خطأ في الخادم" });
    }
...
router.patch<{ id: string }, Notification | { error: string }>(
  "/:id/read",
  async (req, res) => {
    try {
      const userId = req.user?.id;
...
router.patch<{}, { message: string } | { error: string }>(
  "/mark-all-read",
  async (req, res) => {
    try {
      const userId = req.user?.id;
...
// حذف إشعار
router.delete<{ id: string }, { message: string } | { error: string }>(
  "/:id",
  async (req, res) => {
    try {
      const userId = req.user?.id;
...
router.post<{}, Notification | { error: string }, CreateNotificationBody>(
  "/",
  async (req, res) => {
    try {
      const userId = req.user?.id;
