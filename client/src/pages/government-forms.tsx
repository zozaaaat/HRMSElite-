...
export default function GovernmentForms() {
  ...
  const { data: governmentForms = [] } = useQuery({
  ...
  const submitFormMutation = useMutation({
    mutationFn: async (formData: any) => {
      ...
      if (!response.ok) throw new Error('Failed to submit form request');
  ...
  const autoFillMutation = useMutation({
    mutationFn: async (formId: string) => {
      ...
      if (!response.ok) throw new Error('Failed to auto-fill form');
      return response.json();
    },
    onSuccess: (data) => {
      toast({
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{(governmentForms as any[]).length}</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{filteredRequests.filter((r: any) => r.status === 'submitted').length}</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{filteredRequests.filter((r: any) => r.status === 'processing').length}</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{filteredRequests.filter((r: any) => r.status === 'completed').length}</p>
              </div>
                  ...
                  <div className="flex gap-2 pt-2">
                    <Dialog>
                      ...
                      <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
                        <DialogHeader>
                          <DialogTitle className="text-xl">{form.formNameArabic}</DialogTitle>
                        </DialogHeader>
                        <div className="space-y-6 mt-4">
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                              ...
                              <p className="text-sm mt-1">{form.lastUpdated}</p>
                            </div>
                          </div>
                        </div>
                      </DialogContent>
                ...
                <tbody>
                  {filteredRequests.map((request: any) => (
                    <tr key={request.id} className="border-b hover:bg-muted/50">
                      ...
                      <td className="p-4">
                        <p className="text-sm">{new Date(request.submissionDate).toLocaleDateString('ar-SA')}</p>
                      </td>
                      <td className="p-4">
          ...
          {selectedForm && (
            <form onSubmit={(e) => {
              ...
              <div>
                ...
                <Input name="employeeId" placeholder="أدخل رقم الموظف إذا كان النموذج خاص بموظف" />
              </div>
              <div>
                <Label>ملاحظات</Label>
                <Textarea 
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button type="button" variant="secondary" onClick={() => autoFillMutation.mutate(selectedForm.id)}>
