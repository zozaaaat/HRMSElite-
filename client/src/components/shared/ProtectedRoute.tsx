...
export function ProtectedRoute ({
  ...
  useEffect(() => {
    ...
    if (requiredRole && user.role !== toAuthUserRole(requiredRole)) {
      ...
      const defaultFallback = getDashboardRoute(toRoutesUserRole(user.role));
    ...
    if (!canAccessPage(toRoutesUserRole(user.role), pageId)) {

      // توجيه إلى لوحة التحكم المناسبة للدور
      const dashboardPath = getDashboardRoute(toRoutesUserRole(user.role));
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center space-y-4">
          ...
          <button
            onClick={() => setLocation(getDashboardRoute(toRoutesUserRole(user.role)))}
...
export function PermissionGuard ({children, permission, fallback}: PermissionGuardProps) {
  ...
  // التحقق من الصلاحية
  const hasAccess = canAccessPage(toRoutesUserRole(user.role), permission);
...
export function AnyPermissionGuard ({children, permissions, fallback}: AnyPermissionGuardProps) {
  ...
  const hasAnyAccess = permissions.some(permission => canAccessPage(toRoutesUserRole(user.role), permission));
...
export function AllPermissionsGuard ({children, permissions, fallback}: AllPermissionsGuardProps) {
  ...
  const hasAllAccess = permissions.every(permission => canAccessPage(toRoutesUserRole(user.role), permission));
